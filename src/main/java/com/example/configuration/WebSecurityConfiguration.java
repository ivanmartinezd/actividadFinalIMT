...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/listar").hasRole("ADMIN").and().formLogin();
		
		http.authorizeRequests()
				.antMatchers(resources).permitAll() // Se permite el acceso a los 'resources'
				.and().authorizeRequests().antMatchers("/console/**").permitAll() //para que funciones h2-console
